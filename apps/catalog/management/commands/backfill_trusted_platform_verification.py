"""Backfill official platform verification for trusted direct sources."""
...
from apps.catalog.trust import (
    ...
    trusted_platform_verification_backfill,
)
...
class Command(BaseCommand):
    help = (
        "Dry-run/apply backfill for platform_verification_status=official "
        "when trusted direct-source directory URLs prove the platform listing."
    )
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--apply",
            action="store_true",
            help="Persist platform_verification_status=official for matches.",
        )
        parser.add_argument(
            "--include-mcp",
            action="store_true",
            help="Allow apps that also have mcp_registry sources.",
        )
        parser.add_argument(
            "--indent",
            type=int,
    ...
    def handle(self, *args, **options) -> None:
        ...
        result = trusted_platform_verification_backfill(
