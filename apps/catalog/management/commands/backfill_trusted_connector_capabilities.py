"""Backfill safe capability defaults for trusted cloud connectors."""
...
from apps.catalog.trust import (
    ...
    trusted_connector_capabilities_backfill,
)
...
class Command(BaseCommand):
    help = (
        "Dry-run/apply backfill for capability rows that are implied by "
        "trusted official cloud connector directories."
    )
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--apply",
            action="store_true",
            help="Persist inferred capability rows.",
        )
        parser.add_argument(
            "--include-mcp",
            action="store_true",
    ...
    def handle(self, *args, **options) -> None:
        ...
        result = trusted_connector_capabilities_backfill(
