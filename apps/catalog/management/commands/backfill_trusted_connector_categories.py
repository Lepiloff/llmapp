"""Backfill safe categories from trusted connector source payloads."""
...
from apps.catalog.trust import (
    ...
    trusted_connector_categories_backfill,
)
...
class Command(BaseCommand):
    help = (
        "Dry-run/apply backfill for categories explicitly mapped from trusted "
        "official cloud connector directory payloads."
    )
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            "--apply",
            action="store_true",
            help="Persist mapped category links.",
        )
        parser.add_argument(
            "--include-mcp",
            action="store_true",
    ...
    def handle(self, *args, **options) -> None:
        ...
        result = trusted_connector_categories_backfill(
